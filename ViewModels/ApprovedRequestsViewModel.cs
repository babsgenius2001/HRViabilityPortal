...
{
    public class ApprovedRequestsViewModel : ViewModelBase
    {
        ...
        public List<HRFacilityMaster> Get(HRFacilityMaster entity)
        {
            ...
            if (!string.IsNullOrEmpty(Convert.ToString(startDate)) && !string.IsNullOrEmpty(Convert.ToString(endDate)))
            {
                ...
                ret = ret.FindAll(p => p.requestStatus == "Approved_For_Disbursement" && p.requestDate >= startDate && p.requestDate <= endDate);
        ...
        protected List<HRFacilityMaster> CreateData()
        {
            ...
            try
            {
                ...
                {
                    ret = db.HRFacilityMasters.Where(x => x.requestStatus == "Approved_For_Disbursement").ToList();
