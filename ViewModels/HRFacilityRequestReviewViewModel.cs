...
{
    public class HRFacilityRequestReviewViewModel: ViewModelBase
    {
        public HRFacilityRequestReviewViewModel()
        {
            Init();
        }
            ...
            if (ret)
            {
                ...
                try
                {
                    ...
                    using (var db = new HRViabilityPortalEntities())
                    {
                        ...
                        else if (entity.hrApprover1ApproveReject == "Approve")
                        {
                        ...
                        {
                           // queueNo = getLatestQueueNumber();

                            rs.hrApprover1 = UserId + "@jaizbankplc.com";
                            rs.hrApprover1Timestamp = DateTime.Now;
                            rs.hrApprover1ApproveReject = "Queued";
                            rs.requestStatus = "Added_To_Queue";
                            rs.hrApprover1Comment = entity.hrApprover1Comment;
                            //rs.queueId = queueNo;

                            op = entity.facilityName + " Facility Request with Reference No :" + entity.requestReferenceNumber + "  Has been Added To Queue For Now!";

                            ReqHistory history = new ReqHistory();
                            history.ActionDateTime = DateTime.Now;
                            history.Initiator = UserId + "@jaizbankplc.com";
                            history.ActionPerformed = "Facility Request Added To Queue with Reason:" + rs.hrApprover1Comment;
                            history.ReqId = entity.requestReferenceNumber;
                            db.ReqHistories.Add(history);
                            Msg = op;
                            IsValid = true;
                ...
                {
                    sendApproveQueuedRequests(entity.requestReferenceNumber);
                }
            }
            return ret;
        }
        ...
        public void sendApproveQueuedRequests(string refNo)
        {
            try
            {
                string subject = "Facility Request Reviewed & Queued For Later By HR!";
                string dt = DateTime.Now.ToString();               
                ...
                using (var db = new HRViabilityPortalEntities())
                {
                    ...
                    StringBuilder sb = new StringBuilder();
                    StreamReader sr = new StreamReader(System.Web.HttpContext.Current.Server.MapPath("~/HtmlDoc/HRReviewToInitiatorQueued.html"));
                    string line = sr.ReadToEnd();
                    sb.Append(line);
                    sb.Replace("{refNo}", refNo);
                   // sb.Replace("{queueNo}", Convert.ToString(queueNo));
                    sb.Replace("{date}", dt);
                    sb.Replace("{initiator}", initiator);
                    sb.Replace("{hrreviewer}", hrreviewer);
                    sb.Replace("{comments}", reason);
        ...
        public int getLatestQueueNumber()
        {
            int nextSeq = 0;

            string query = "select HRVIABILITY_SEQ.nextval as nextvalue from dual";
            try
            {
                using (OracleConnection connection = new OracleConnection(ImalConn_))
                {
                    ...
                    while (reader.Read())
                    {
                        nextSeq = Convert.ToInt32(reader["nextvalue"]);
                    }
        ...
        protected void GetDropDownItems()
        {
            ...
            SelectList ItemE3 = new SelectList();
            ItemE3.Text = "Add To Queue"; ItemE3.Value = "1";
            List.Add(ItemE);
        }
